...
const FacultyDashboard: React.FC = () => {
  ...
  const statCards = [
    { label: "Today's Classes", value: stats?.today_classes_count || 0, icon: Calendar, border: 'stat-card-border-blue', iconBg: 'hsl(215 90% 50% / 0.1)', iconColor: 'hsl(215, 90%, 50%)' },
    { label: 'Total Subjects', value: stats?.total_subjects || 0, icon: BookOpen, border: 'stat-card-border-green', iconBg: 'hsl(155 70% 38% / 0.1)', iconColor: 'hsl(155, 70%, 38%)' },
    { label: 'Weekly Classes', value: stats?.weekly_classes_count || 0, icon: Clock, border: 'stat-card-border-purple', iconBg: 'hsl(265 65% 50% / 0.1)', iconColor: 'hsl(265, 65%, 50%)' },
  ];
  ...
  return (
    <div>
      ...
      <div className="info-banner mb-8">
        <span className="text-xs font-bold uppercase tracking-widest" style={{ color: 'hsl(42, 95%, 55%)' }}>Faculty Portal</span>
        <h2 className="mt-2">Welcome, {user?.full_name.split(' ')[0]} 👋</h2>
        <p>Here's your teaching schedule and overview for the week.</p>
      </div>
      ...
      <div className="flex items-center gap-3 mb-5">
        ...
        <h3 className="font-heading font-extrabold text-base tracking-tight text-foreground">Today's Schedule</h3>
      </div>
          ...
          {stats.today_timetable.map((entry: any) => (
            <div key={entry.id} className="timetable-card">
              ...
              <div className="mt-3 space-y-2 text-sm text-muted-foreground">
                ...
                <p className="flex items-center gap-2">📚 <span>Sem {entry.semester} · Sec {entry.section}</span></p>
              </div>
              <div className="mt-3">
